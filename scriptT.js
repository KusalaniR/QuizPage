 ...
 const timeText = document.querySelector(".timer .time_Out_txt");
...
restart_quiz.onclick = ()=>{
    ...
    timeText.textContent = "Time Out"; //change the text of timeText to Time Out
...
function showResult(){
    ...
    if (userScore > 5){
        ...
        let scoreTag = ' <p>You got only '+ userScore +' out of '+ questions.length +'</p>';
        scoreText.innerHTML = scoreTag;
    }
}
...
function queCounter(index){
    ...
    let totalQueCounTag = '<span>'+ index +'<p> of </p>'+ questions.length +'</p> Questions</span>';
...
function displayClue() {
  ...
  const clueImage = document.querySelector("#clue-image");
  const clueDescription = document.querySelector("#clue-description");
  const correctAnswer = questions[currentQuestion].answer;
  
}
let percentageScore = (score / questions.length) * 100;
console.log(`Your score: ${percentageScore.toFixed(2)}%`);
